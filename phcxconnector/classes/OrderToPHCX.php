...
class OrderToPHCX extends Module
{
    ...
    public static function sendShippingCost($session, $client, $shipping, $getDocTypeShip, $docID)
    {
        ...
        $shipping_reference = Configuration::get('PHCXCONNECTOR_SHIPPINGCOST');
        ...
        $result = ProductToKeyInvoice::upsertProduct(
            "$Ref",
            "$Name",
            "$ShortName",
            "$TAX",
    ...
    public static function sendOrderToKeyInvoice($id_order, $from)
    {
        if (Validate::isLoadedObject($order = new Order($id_order))) {
            ...
            if ($from == 'hookDisplayAdminOrder') {
                ...
                $getDocTypeShip = Tools::getValue('PHCXCONNECTOR_SHIP_DOC_TYPE');

            } else {
  
                $getDocTypeShip = Configuration::get('PHCXCONNECTOR_SHIP_DOC_TYPE');
            }
            ...
            if (empty($shipping_reference)) {
                return array(-969,
                    ...
                    configurada no KeyInvoice Connector! Encomenda n&atilde;o sincronizada!"
                );
            }
            ...
            //$getDocTypeInv  = Tools::getValue('PHCXCONNECTOR_INV_DOC_TYPE');
            ...
            // create document
            $result = $client->insertDocumentHeader(
                "$session",
                "$vat_number",
                "$getDocTypeShip",
                "",
            ...
            if ($getDiscounts) {
                return array(-969,
                    ...
                    nesta encomenda no KeyInvoice Connector!"
                );
            }
            return $result;
        }
